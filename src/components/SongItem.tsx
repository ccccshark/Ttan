  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isActive && "bg-accent/10 dark:bg-accent/12"
      )}
    >
      {/* 序号或封面 */}
      {showIndex && typeof index === "number" ? (
        <div className="flex h-12 w-8 items-center justify-center text-sm text-ink-subtle dark:text-white/50">
          {isActive && isPlaying ? (
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-truncate mt-0.5 text-xs text-ink-muted dark:text-white/60">
          {song.artist}
          {song.album && song.album !== "未知" && (
            <>
              <span className="mx-1 text-ink-subtle dark:text-white/40">·</span>
              {song.album}
            </>
          )}
        </div>
      ...
      {/* 时长 */}
      {song.duration > 0 && (
        <div className="text-xs tabular-nums text-ink-subtle dark:text-white/50">
      ...
      {onMore && (
        <button
          ...
          aria-label="更多操作"
          className="flex h-9 w-9 items-center justify-center rounded-full text-ink-subtle transition-colors hover:bg-black/5 hover:text-ink dark:text-white/50 dark:hover:bg-white/10 dark:hover:text-white"
        >
